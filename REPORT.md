...
```bash
# Reach K8s node at 172.19.0.35
rawSql: "SELECT * FROM dblink('host=172.19.0.35 port=31222 user=postgres
...
# Reach archive host at 172.19.64.16
rawSql: "SELECT * FROM dblink('host=172.19.64.16 port=25432 user=postgres
         password=postgres dbname=postgres', 'SELECT version()') AS t(v text)"
...
The archive host (`172.19.64.16:25432`) runs the Hasura GraphQL engine. Its
metadata catalog stores connection strings for **all 10 data sources** that
feed app.bifrost.io.

```bash
rawSql: "SELECT * FROM dblink(
    'host=172.19.64.16 port=25432 user=postgres password=postgres dbname=bifrost_kusama',
...
|---|-------------|------------|------------|
| 1 | **bifrost_monitor** | `postgres:polkadot-bnc-123@172.19.0.35:30423` | **polkassembly_post**, treasury_info, bbbnc_info |
| 2 | **bifrost_kusama_squid** | `postgres:postgres@172.19.0.35:31224/squid` | **slp_ratio** (vKSM/KSM rate) |
| 3 | **bifrost_polkadot_squid** | `postgres:postgres@172.19.0.35:31223/squid` | **slp_polkadot_ratio** (vDOT/DOT rate) |
| 4 | **slp_veth** | `postgres:postgres@172.19.0.35:31222/squid` | **slp_ratio** (vETH/ETH rate) |
| 5 | **bifrost_vpha_squid** | `postgres:postgres@172.19.0.35:31235/squid` | **slp_ratio** (vPHA/PHA rate) |
| 6 | **monitor-3-polkadot** | `postgres:postgres@172.19.0.35:31230/squid` | **lend_market_apy** |
| 7 | **monitor-3-kusama** | `postgres:postgres@172.19.0.35:31233/squid` | **lend_market_apy** |
| 8 | **bifrost-vercel-api** | `postgres:postgres@43.154.29.23:31232` | new_stats |
| 9 | **bifrost_kusama** | env var (local 172.19.64.16) | block, call, event, extrinsic |
...
rawSql: "SELECT * FROM dblink(
    'host=172.19.0.35 port=30423 user=postgres password=polkadot-bnc-123 dbname=postgres',
...
│     └─ dblink → 172.19.64.16:25432 (archive host)
│        └─ hdb_catalog.hdb_metadata (Hasura metadata)
│           └─ 10 data source credentials extracted
│              └─ dblink → 172.19.0.35:30423 (bifrost_monitor)
