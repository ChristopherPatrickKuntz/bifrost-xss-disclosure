...
echo -e "\n${YELLOW}Step 5: Reach K8s node via dblink (172.19.0.35)${NC}"
K8S_VER=$(sql_query "SELECT * FROM dblink('host=172.19.0.35 port=31222 user=postgres password=postgres dbname=postgres', 'SELECT version()') AS t(v text)" | extract_first)
K8S_SHORT=$(echo "$K8S_VER" | head -c 60)
check "dblink to K8s node (port 31222) succeeds" "$K8S_VER"
echo "    PostgreSQL: ${K8S_SHORT}..."

echo -e "\n${YELLOW}Step 6: Reach archive host via dblink (172.19.64.16)${NC}"
ARCHIVE_VER=$(sql_query "SELECT * FROM dblink('host=172.19.64.16 port=25432 user=postgres password=postgres dbname=postgres', 'SELECT version()') AS t(v text)" | extract_first)
...
HASURA_META=$(sql_query "SELECT * FROM dblink('host=172.19.64.16 port=25432 user=postgres password=postgres dbname=bifrost_kusama', 'SELECT metadata::text FROM hdb_catalog.hdb_metadata LIMIT 1') AS t(meta text)" | extract_first)
...
MONITOR_DBLINK="host=172.19.0.35 port=30423 user=postgres password=polkadot-bnc-123 dbname=postgres"
...
echo -e "${BOLD}║        → dblink to archive host (172.19.64.16)                     ║${NC}"
